SELECT_PRODUCT_WITH_PAGES_AND_STATES = 'SELECT p.id, p.name, p.description, p.category,  p.description, p.preview_url, ' \
                                       'marketplaces_instock.count as marketplaces_count_instock, ' \
                                       'MIN(ps.price) as min_offer, ' \
                                       ...
                                       'JOIN (SELECT p.id, count(DISTINCT pp.marketplace_id) AS count ' \
