    ...
    switch (action.type) {
        ...
        case SET_CURRENT_PRODUCT:{
            if (state.results[0].id === 0){
                ...
                console.log({...state, results: [action.data]})
                return {...state, results: [action.data]}
            }
            if (action.data.id === 0) return initialState
            let copyResults = state.results.map(item=>(item.id === action.data.id
                ?
                {...item,id:action.data.id,
                category:action.data.category, description:action.data.description, name:action.data.name,
                preview_url:action.data.preview_url, product_pages:action.data.product_pages}
                :
                item))
            console.log({...state, results:copyResults})
